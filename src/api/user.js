...
export const getSignInUrlAPI = () => new Promise((resolve, reject) => {
  http.get(`${globalContext.backendUrl}/account/oauth/login/`).then((res) => resolve(res), (err) => reject(err));
});

export const signInAPI = (data) => new Promise((resolve, reject) => {
  http.post(`${globalContext.backendUrl}/account/oauth/callback/`, data).then((res) => resolve(res), (err) => reject(err));
});
